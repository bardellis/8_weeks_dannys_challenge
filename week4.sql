...
select count(*) as deposits, sum(txn_amount) as amounts
FROM customer_transactions;

-- What is the average total historical deposit counts and amounts for all customers?
select count(*) as deposits, sum(txn_amount_val) as amounts 
FROM (select 
	c.customer_id,
	c.start_date, 
    c.end_date, 
    r.region_name,
	case when txn_date >=start_date and txn_date <= end_date
    then txn_date 
    else null 
    end as txn_date_val,
   	case when txn_date >=start_date and txn_date <= end_date
    ...
    end as txn_amount_val
FROM customer_nodes AS c
JOIN regions AS r ON c.region_id = r.region_id
JOIN customer_transactions as t on c.customer_id= t.customer_id) subquery
where txn_date_val is not null;
